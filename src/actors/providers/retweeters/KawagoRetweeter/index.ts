...
import { KAWAGO_ACCOUNT } from '../../../../constants/accounts';
import { Tweet } from '../../../fechers/TweetFetcher/types';
import { BaseRetweeter } from '../BaseRetweeter';
import { isSameDay } from '../../../../utils/date';

export class KawagoRetweeter extends BaseRetweeter {
  constructor(twitter: Twit) {
    super({ twitter, watchingAccounts: [KAWAGO_ACCOUNT] });
  }
  ...
  private isInformation(tweet: Tweet, today?: Date): boolean {
    ...
    if (text !== undefined) {
      if (text.includes('お知らせ') && (today !== undefined ? isSameDay(today, createdDate) : true)) {
        return true;
      }
    }
