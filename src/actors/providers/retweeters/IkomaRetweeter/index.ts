...
import { IKOMA_ACCOUNT } from '../../../../constants/accounts';
import { Tweet } from '../../../fechers/TweetFetcher/types';
import { BaseRetweeter } from '../BaseRetweeter';
import { isSameDay } from '../../../../utils/date';

export class IkomaRetweeter extends BaseRetweeter {
  constructor(twitter: Twit) {
    super({ twitter, watchingAccounts: [IKOMA_ACCOUNT] });
  }
  ...
  private isInformation(tweet: Tweet, today?: Date): boolean {
    ...
    if (text !== undefined) {
      if (text.includes('出演情報') && (today !== undefined ? isSameDay(today, createdDate) : true)) {
        return true;
      }
    }
